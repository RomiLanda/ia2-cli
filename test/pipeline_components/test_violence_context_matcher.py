...
class ViolenceContextMatcherTest(unittest.TestCase):
    ...
    def test_a_custom_entity_pipeline_detects_violence_contexts(self):
        entity_left_nbors = violence_nbors + list(
            map(lambda nbor: f"{nbor}:", violence_nbors)
        )
        # We take away a few elements of the remaining lists to avoid running very long tests
        _two_words_violence = list(
            map(
                lambda words: f"{words[0]} y {words[1]}",
                itertools.combinations(violence_types, 2),
            )
        )
        two_words_violence = list(
            itertools.islice(
                _two_words_violence, int(len(_two_words_violence) / 3)
            )
        )
        three_words_violence = list(
            itertools.islice(
                map(
                    lambda words: f"{words[0]}, {words[1]} y {words[2]}",
                    itertools.combinations(violence_types, 3),
                ),
        ...
        entity_values = list(
            map(
                lambda v: (v, len(self.nlp(v))),
                violence_types + two_words_violence + three_words_violence,
            )
        )
        base_test_sentences = [
            (
        ...
        test_sentences = generate_fake_sentences(
            entity_values, base_test_sentences
        )
        ...
        for (
            ...
            for left_nbor_word in entity_left_nbors:
                ...
                self.assertEqual(
                    expected_span.text, f"{left_nbor_word} {entity_value}"
                )
    ...
    def test_a_custom_entity_pipeline_detects_gender_violence_contexts(self):
        ...
        # A list of tuple where the first component is an article value and the
        # second the value length in spacy terms.
        parsed_gender_violence_types = list(
            map(lambda word: f"de {word}", gender_violence_types)
        )
        entity_values = list(
            map(lambda v: (v, len(self.nlp(v))), parsed_gender_violence_types)
        )
        base_test_sentences = [
            (
                26,
